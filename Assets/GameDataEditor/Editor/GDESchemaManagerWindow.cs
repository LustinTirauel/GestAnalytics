...
namespace GameDataEditor
{
	public class GDESchemaManagerWindow : GDEManagerWindowBase {
	    ...
	    protected override void DrawCreateSection()
	    {
	        ...
	        // Draw rate box        
	        content.text = GDEConstants.BuyLinkText;
	    ...
	    private void DrawAddFieldSection(string schemaKey, Dictionary<string, object> schemaData)
	    {
	        ...
	        GUIContent newFieldLabelContent = new GUIContent(GDEConstants.NewFieldHeader);
	        float width = labelStyle.CalcSize(newFieldLabelContent).x;
	        GUI.Label(new Rect(currentLinePosition, TopOfLine(), width, StandardHeight()), newFieldLabelContent, labelStyle);
	        ...
	        GUIContent content = new GUIContent(GDEConstants.AddFieldBtn);
	        width = GUI.skin.button.CalcSize(content).x;
	        if (GUI.Button(new Rect(currentLinePosition, TopOfLine(), width, StandardHeight()), content))
	        {
